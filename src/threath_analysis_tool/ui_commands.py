...
class AddNodeCommand(Command):
    def __init__(self, graph: Graph, node_data: Dict[str, Any]):
        ...
        if self.node_data['type'] == 'Actor':
            ...
            if has_self_trigger: self.node.triggers.append(SelfTrigger())
...
class EditNodeCommand(Command):
    ...
    def execute(self):
        ...
        if isinstance(node, Actor):
            new_triggers = [t for t in node.triggers if isinstance(t, CommunicationTrigger)]
            if self.new_data.get('has_self_trigger'): new_triggers.append(SelfTrigger())
    ...
    def description(self) -> str:
        if self.node_before.name != self.new_data['name']: return f"Rename Node: '{self.node_before.name}' → '{self.new_data['name']}'"
...
class DeleteNodeCommand(Command):
    def __init__(self, graph: Graph, node_id: str):
        ...
        self.deleted_edges = [e for e in graph.edges if e.source == node_id or e.target == node_id]
        self.modified_actors_before: Dict[str, Actor] = {}
        if self.node:
            for actor in graph.nodes:
                if isinstance(actor, Actor):
                    triggers_to_remove = []
                    if self.node.type == 'Actor' and actor.id != self.node.id:
                        triggers_to_remove = [t for t in actor.triggers if isinstance(t, CommunicationTrigger) and t.source_actor_id == self.node_id]
    ...
    def execute(self):
        ...
        for actor_id in self.modified_actors_before:
            ...
            if actor:
                if self.node.type == 'Actor': actor.triggers = [t for t in actor.triggers if not (isinstance(t, CommunicationTrigger) and t.source_actor_id == self.node_id)]
    ...
    def undo(self):
        ...
        for actor_id, original_state in self.modified_actors_before.items():
            for i, n in enumerate(self.graph.nodes):
                if n.id == actor_id: self.graph.nodes[i] = original_state; break
    @property
...
class AddEdgeCommand(Command):
    def __init__(self, graph: Graph, edge_data: Dict[str, Any]):
        ...
        if not source_node or not target_node: raise ValueError("Edge source or target not found.")
        err = None
        if edge_type == 'read' and not (source_node.type == 'Datasource' and target_node.type == 'Actor'): err = "Invalid 'read' edge: Must be from a Datasource to an Actor."
        elif edge_type == 'write' and not (source_node.type == 'Actor' and target_node.type == 'Datasource'): err = "Invalid 'write' edge: Must be from an Actor to a Datasource."
        elif edge_type in ['communicate', 'respond'] and not (source_node.type == 'Actor' and target_node.type == 'Actor'): err = f"Invalid '{edge_type}' edge: Must be between two Actors."
    ...
    def undo(self):
        ...
        if self.target_actor_before:
            for i, n in enumerate(self.graph.nodes):
                if n.id == self.target_actor_before.id: self.graph.nodes[i] = self.target_actor_before; break
    @property
    def description(self) -> str:
