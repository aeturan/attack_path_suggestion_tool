...
from domain import Actor, DatasourceTrigger, SelfTrigger
    ...
    with st.sidebar:
        ...
        with st.form("new_session_form", clear_on_submit=True):
            new_graph_name = st.text_input(
                "New Graph Name", placeholder="e.g., In-Car Agent System"
            )
        ...
        if st.session_state.graph.id:
            ...
            if st.session_state.confirming_delete:
                ...
                def show_confirm_dialog():
                    st.warning(
                        f"Are you sure you want to permanently delete the graph '{st.session_state.graph.name}'?"
                    )
        ...
        col1, col2 = st.columns(2)
        if col1.button(
            "Undo", use_container_width=True, disabled=not st.session_state.history.undo_stack
        ):
            st.session_state.history.undo()
            save_current_session()
            st.rerun()
        if col2.button(
            "Redo", use_container_width=True, disabled=not st.session_state.history.redo_stack
        ):
        ...
        with st.expander(
            "➕ Add New Element", expanded=st.session_state.add_expander_state
        ):
        ...
        with st.expander(
            "✏️ Manage Elements", expanded=st.session_state.manage_expander_state
        ):
...
def render_add_node_form():
    ...
    node_type = st.radio(
        "Node Type", ["Actor", "Datasource"], horizontal=True, key="add_node_type"
    )
    ...
    if node_type == "Actor":
        ...
        st.caption(
            "An Actor can be triggered in three ways: by itself (`self-trigger`), by a change to a datasource it `watches`, or automatically by an incoming `communicate` or `respond` edge."
        )

        has_self_trigger = st.checkbox(
            "Can self-trigger?", help="Allows the actor to initiate actions on its own."
        )
...
def render_add_edge_workflow():
    ...
    if st.session_state.edge_creation_source_id is None:
        ...
        source_node = st.session_state.graph.get_node(
            st.session_state.edge_creation_source_id
        )
        if not source_node:
            st.session_state.edge_creation_source_id = None
            st.rerun()
        ...
        target_id = st.selectbox(
            ...
            format_func=lambda x: target_options.get(x, "Choose..."),
            index=0,
        )
        if target_id:
...
def render_manage_node_workflow():
    ...
    if selected_id:
        ...
        if col1.button("Save Changes", use_container_width=True, type="primary"):
            ...
            command = EditNodeCommand(st.session_state.graph, node.id, new_data)
            execute_command(command)

        if col2.button("Delete Node", use_container_width=True):
            st.session_state.manage_expander_state = True
            st.session_state.node_to_delete = node
            st.rerun()
